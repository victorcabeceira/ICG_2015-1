...
bool CubemapTexture::Load()
{
    ...
    for (unsigned int i = 0 ; i < ARRAY_SIZE_IN_ELEMENTS(types) ; i++) {
	    ...
	    m_pImage = IMG_Load(m_fileNames[i].c_str());
	    if(m_pImage == NULL){
		printf("\n ERRO: Erro ao ler a imagem de textura %s\n %s\n\n", m_fileNames[i].c_str(), SDL_GetError());
	    }
	    printf("\n arquivo lido: %s\n",m_fileNames[i].c_str());
	   int Mode = GL_BGR;
	   //int Mode = GL_RGB;
	   if(m_pImage->format->BytesPerPixel == 4) {
        ...
        glTexImage2D(types[i], 0, GL_RGB, m_pImage->w, m_pImage->h, 0, Mode, GL_UNSIGNED_BYTE,m_pImage->pixels);
